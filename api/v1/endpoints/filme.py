...
from models.filme_models import FilmeModel
from schemas.filme_schemas import FilmeSchema
...
async def post_filme(filme: FilmeSchema, db: AsyncSession = Depends(get_session)):
    novo_filme = FilmeModel(titulo=filme.titulo, duracao=filme.duracao)
...
async def get_filme(filme_id: int, db: AsyncSession = Depends(get_session)):
    async with db as session:
        ...
        if filme:
            ...
            raise HTTPException(detail='Filme não cadastrado.',
...
async def put_filme(filme_id: int, filme: FilmeSchema, db: AsyncSession = Depends(get_session)):
    async with db as session:
        ...
        if filme_up:
            ...
            filme_up.duracao = filme.duracao
            
            await session.commit()
            
            return filme_up
        else:
            raise HTTPException(detail='Filme não cadastrado.', status_code=status.HTTP_404_NOT_FOUND)
